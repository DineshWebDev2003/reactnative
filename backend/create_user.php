...
try {
    ...
    $sql = "INSERT INTO users (name, role, branch, email, mobile, password, childName, childClass, student_id, staff_id, fee_due, franchisee_share) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $result = $stmt->execute([
        $name, $role, $branch, $email, $mobile, $hashed_password, $childName, $childClass,
        $student_id, $staff_id, $fee_due, $franchisee_share
    ]);
