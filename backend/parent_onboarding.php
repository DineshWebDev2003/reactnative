...
try {
    $sql = "UPDATE users SET onboarding_complete=1, child_photo=?, father_photo=?, mother_photo=?, guardian_photo=? WHERE id=?";
    $stmt = $pdo->prepare($sql);
    $result = $stmt->execute([$child_path, $father_path, $mother_path, $guardian_path, $user_id]);
    if ($result) {
