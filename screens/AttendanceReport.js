...
export default function AttendanceReport({ navigation }) {
  ...
  const fetchAttendance = () => {
    setLoading(true);
    fetch(`${BASE_URL}/get_attendance.php?branch=${encodeURIComponent(selectedBranch)}&date=${date.toISOString().slice(0,10)}`)
      .then(res => res.json())
      .then(data => {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.row}>
              <Text style={styles.cell}>{item.name || item.childName}</Text>
